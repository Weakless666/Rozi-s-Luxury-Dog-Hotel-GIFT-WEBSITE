...
const ChatBot = () => {
  ...
  const quickQuestions: QuickQuestion[] = [
    ...
    },
    {
      ...
      answer: "Да! Предлагаме професионални груминг услуги - къпане и ресане. Цените започват от 15€. Къпане: малки кучета (до 5кг) 20€, средни 25€, големи 30€, гигантски 35€. Ресане: малки 20€, средни 25€, големи 30€, гигантски 35€. Всички услуги се извършват от опитни майстори! ✂️"
    },
    {
      id: 10,
    ...
    },
    {
      ...
      answer: "Цената е 55€ на нощувка и включва всички основни услуги - настаняване, хранене, игри, наблюдение и грижа. Груминг услугите са допълнителни: къпане от 20-35€, ресане от 20-35€ в зависимост от размера на кучето! 💰"
    },
    {
      id: 55,
    ...
    },
    {
      ...
      answer: "Имаме детайлни цени за груминг услуги: КЪПАНЕ - малки кучета (до 5кг) 20€, средни (до 15кг) 25€, големи (до 40кг) 30€, гигантски (над 40кг) 35€. РЕСАНЕ - малки 20€, средни 25€, големи 30€, гигантски 35€. ✂️"
    },
    {
      id: 58,
    ...
    },
    {
      ...
      answer: "Приемаме нашите гости от 8:00 до 20:00."
    },
    {
      id: 64,
  ...
  // Видими въпроси (скриваме някои от списъка с бързи въпроси)
  const hiddenQuestionIds = [
    2,  // Приемате ли всички породи?
    3,  // Колко дни предварително да резервирам?
    4,  // Имате ли свободни места за уикенда?
    5,  // Какво включва престоят?
    7,  // Колко пъти на ден ги храните?
    8,  // Следите ли специални режими?
    9,  // Предлагате ли къпане и груминг?
    10, // Може ли да давате лекарства?
    12, // Има ли видеонаблюдение?
    13, // Има ли двор за игри?
    14, // Колко често ги извеждате навън?
    15, // Приемате ли несоциални кучета?
    16, // Колко струва престоят на ден?
    17, // Имате ли отстъпки за дълъг престой?
    18, // Какви методи на плащане приемате?
    19, // Какво се случва ако закъснея?
    20, // Къде се намирате точно?
    21, // Работите ли през уикендите?
    22, // Приемате ли само кучета или и котки?
    29, // Приемате ли малки кученца (под 6 месеца)?
    30, // Има ли ограничение в килограмите?
    31, // Колко животни приемате максимум на ден?
    32, // Колко служители се грижат за животните?
    33, // Кучето ми не е кастрирано - това проблем ли е?
    34, // Трябва ли кучето да има паспорт?
    35, // Изисквате ли ваксини и обезпаразитяване?
    36, // Какво меню предлагате за хранене?
    37, // Мога ли да донеса собствена храна и купички?
    38, // Предлагате ли храна за чувствителни кучета?
    39, // Какво става ако кучето не иска да яде?
    40, // Може ли персоналът да дава лекарства по график?
    41, // Имате ли ветеринар на място?
    42, // Животните спят заедно или поотделно?
    43, // Как изглеждат стаите?
    44, // Температурата се регулира ли?
    45, // Колко често се извеждат навън?
    46, // Има ли разходки извън двора?
    50, // Ще получавам ли снимки всеки ден?
    51, // Мога ли да се обаждам да питам как е кучето?
    52, // Изпращате ли актуализации по Viber?
    53, // Може ли да направя видеоразговор с кучето?
    54, // Каква е цената на нощувката?
    55, // Какво включва цената?
    59, // Предлагате ли отстъпки за повече от едно куче?
    60, // Колко струва транспортът?
    61, // Как се приема кучето?
    65, // Какво става ако закъснея с прибирането?
    66, // Кучето ми не се разбира с други - ще го приемете ли?
    67, // Има ли обучение за социализация?
    68, // Какво става ако се сбие с друго куче?
    69, // Кучето ми лае много - това проблем ли е?
    70, // Предлагате ли тестов престой?
    71, // Имате ли груминг услуги?
    72, // Може ли кучето да бъде снимано професионално?
    73, // Имате ли транспорт от/до София?
    74, // Предлагате ли дневна грижа (dog daycare)?
    47, // Предлагате ли индивидуално внимание?
    48, // Кучето ми е страхливо - ще се чувства ли добре?
    49, // Как следите дали кучетата се разбират?
    56, // Има ли различни пакети?
    57  // Колко струва къпането и подстригването?
  ]
  const visibleQuestions = quickQuestions.filter((q) => !hiddenQuestionIds.includes(q.id))

  // Филтриране на въпросите според търсенето (само сред видимите)
  const filteredQuestions = visibleQuestions.filter((question) =>
    question.question.toLowerCase().includes(searchQuery.toLowerCase()) ||
    question.answer.toLowerCase().includes(searchQuery.toLowerCase())
  )

  const topQuestions = visibleQuestions.filter((q) => [1, 6, 11].includes(q.id))
  const otherQuestions = visibleQuestions.filter((q) => !topQuestions.some((t) => t.id === q.id))
  useEffect(() => {

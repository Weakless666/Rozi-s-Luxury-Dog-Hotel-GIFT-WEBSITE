...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fullMessage = `
Служба: ${formData.service || 'Не е избрана'}
${formData.checkIn ? `Настаняване: ${formData.checkIn}` : ''}
${formData.checkOut ? `Напускане: ${formData.checkOut}` : ''}
${formData.dogName ? `Име на кучето: ${formData.dogName}` : ''}
${formData.dogBreed ? `Порода: ${formData.dogBreed}` : ''}
${formData.dogAge ? `Възраст: ${formData.dogAge}` : ''}
